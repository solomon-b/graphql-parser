...
action_0,
 ...
 action_2,
 action_3,
 action_4 :: () => Prelude.Int -> ({-HappyReduction (Parser) = -}
	   Prelude.Int 
	-> (Token)
...
happyReduce_1 :: () => ({-HappyReduction (Parser) = -}
	   Prelude.Int 
	-> (Token)
	-> HappyState (Token) (HappyStk HappyAbsSyn -> [(Token)] -> (Parser) HappyAbsSyn)
	-> [HappyState (Token) (HappyStk HappyAbsSyn -> [(Token)] -> (Parser) HappyAbsSyn)] 
...
happyExpList = Happy_Data_Array.listArray (0,16) ([2048,2048,4096,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parser","expr","number","int","'true'","'false'","string","ident","':'","'{'","'}'","'('","')'","%eof"]
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	TokNumLit _ happy_dollar_dollar -> cont 5;
	TokIntLit _ happy_dollar_dollar -> cont 6;
	TokBoolLit happy_dollar_dollar -> cont 7;
	TokBoolLit happy_dollar_dollar -> cont 8;
	TokStringLit happy_dollar_dollar -> cont 9;
	TokIdentifier happy_dollar_dollar -> cont 10;
	TokSymbol (Loc happy_dollar_dollar SymColon) -> cont 11;
	TokSymbol (Loc happy_dollar_dollar SymCurlyOpen) -> cont 12;
	TokSymbol (Loc happy_dollar_dollar SymCurlyClose) -> cont 13;
	TokSymbol (Loc happy_dollar_dollar SymParenOpen) -> cont 14;
	TokSymbol (Loc happy_dollar_dollar SymParenClose) -> cont 15;
	_ -> happyError' ((tk:tks), [])
	}
